...
//	var_dump($classData);die;
	...
	if($xyData["zgyq"]==1){
		...
		{
			$bw->msg('您不符合家教要求，请关注其他订单信息！原因为您的职位类型不是大学生',"students.php");
			exit;
		}
	}
	if($xyData["zgyq"]==2){
		if($classData["levels"]!=2)
		{
			$bw->msg('您不符合家教要求，请关注其他订单信息！原因为您的职位类型不是：职业教师',"students.php");
			exit;
		}
	}
	if($xyData["zgyq"]==3){
		if($classData["levels"]!=3)
		{
			$bw->msg('您不符合家教要求，请关注其他订单信息！原因为您的职位类型不是：留学、海归',"students.php");
			exit;
		}
	}
	if($xyData["zgyq"]==5)
	{
		...
		{
			$bw->msg('您不符合家教要求，请关注其他订单信息！原因为您的职位类型不是：明星在职教师',"students.php");
				exit;
		}
	}
	if($xyData["zgyq"]==6)
	{
		...
		{
			$bw->msg('您不符合家教要求，请关注其他订单信息！原因为您的职位类型不是：明星大学生',"students.php");
				exit;
		}
...
//	if($xyData['zgyq'] == '' || $xyData['zgyq'] ==4){
//
//	}
	if($xyData['jysex'] != $sex && $xyData['jysex'] !='不限'){

		$bw->msg('您不符合家教要求，原因为：性别不符合',"students.php");
				exit;
