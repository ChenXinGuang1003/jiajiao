...
<div class="zhifu_content">
     ...
     if($_GET["act"]=="name")
	 {
		...
		if($bw->selectOnly("*","bw_qjj","name='".$_POST["username"]."'"))
		{
			$a=1;
		}else{
			$bw->msg("没有找到您所输入的账号","student_getpwd.php");
			}
	 }
     if($_GET["act"]=="email")
	 {
		 ...
		 {
			 $bw->msg("账号不能为空","student_getpwd.php");
		 }
		if($umember=$bw->selectOnly("*","bw_qjj","name='".$_POST["uname"]."'"))
		{
			$a=1;
		  	//die($umember["email"]);
		...
		$mailsubject=@iconv("UTF-8", "gb2312", "欢迎使用武汉才殊家教网");//如果你页面为UTF-8，这里还要转码一下
		$mima=rand(10000000,99999999);
		$pwd=md5($mima);
		$mailbody = "尊敬的会员,欢迎您注册武汉才殊家教网家教网(<a href='http://www.caishujy.com' target='_blank'>www.caishujy.com</a>)<br>请妥善保留这封电子邮件. 您的帐号资料如下<br><br>登陆帐号:&nbsp;<span style='color:#ff0000'>".$_POST["uname"]."</span><br>您登陆的新密码:&nbsp;<span style='color:#ff0000'>".$mima."</span><br><br>请妥善保管.";    //邮件内容
		...
		$xgsql="update bw_qjj set password='".$pwd."' where name='".$_POST["uname"]."'";
	    $bw->query($xgsql);
