...
if($_GET["act"]=="hy")
{
    ...
    if(!empty($_POST["kfdfs"]))
    {
		...
		if($bw->insert('bw_member', $teacherData))
		{
			...
			$mailbody = "尊敬的会员,欢迎您注册武汉才殊家教网(<a href='http://www.caishujy.com' target='_blank'>http://www.caishujy.com</a>)<br>请妥善保留这封电子邮件. 您的帐号资料如下<br><br>登陆帐号:&nbsp;<span style='color:#ff0000'>".$_POST["username"]."</span><br>登陆密码:&nbsp;<span style='color:#ff0000'>".$mima."</span><br><br>请妥善保管.";    //邮件内容
			...
			$sql = "UPDATE bw_member SET dlcs = dlcs+1,reg_action='1',zjtime='".date("y-m-d H:i:s")."' WHERE id = ".$_SESSION["userid"];//登录次数,登录时间
			$bw->query($sql);
...
学生上门<span style="color:#F00">&nbsp;</span></td>
                          ...
                          <tr>
                            ...
                            <td height="159" colspan="5"><textarea name="xsyq" id="xsyq" cols="45" rows="5" style="width:500px; line-height:20px;">执行武汉上门家教网薪水标准。</textarea>
		...
		<option value="其他搜索引擎">其他搜索引擎</option>
                <option value="校园宣传广告">校园宣传广告</option>	
		...
		<option value="其他">其他</option>
                           ...
                           <input type="hidden" name="levels" id="levels" value="<?php echo $_POST["levels"];?>">
								<input type="hidden" name="reg_action" id="reg_action" value="1" />
