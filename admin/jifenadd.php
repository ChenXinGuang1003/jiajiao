...
if($action == 'insert')
{
	if($userData = $bw->selectOnly('id, jifen', 'bw_member', "username = '{$_POST['username']}'"))
	{
      if (strpos($_POST['jifen'], '+') !== FALSE) {
        // var_dump("+");
          $userData['jifen'] = $userData['jifen'] + substr($_POST['jifen'], 1);
      } else if (strpos($_POST['jifen'], '-') !== FALSE) {
        // var_dump("-");
          $userData['jifen'] = $userData['jifen'] - substr($_POST['jifen'], 1);
      } else {
        // var_dump("else");
      ...
      $sql = "UPDATE bw_member SET sd_jifen_reason=".$_POST['jflog'].",jifen = '" . $userData['jifen'] . "' WHERE id = " . $userData['id'];
      $sql2 = "insert into  jifen_reason ( username,jifen,reason )VALUES ('".$_POST['username']."','".$_POST['jifen']."','".$_POST['jflog']."')";
        $bw->query($sql2);
...
<table width="788" cellspacing="0" cellpadding="0">
    <tr>
        <td width="152" align="right">会员名：</td>
        <td width="10">&nbsp;</td>
