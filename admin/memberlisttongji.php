...
//删除多条数据
	...
	if(isset($_POST['id']))
	{
		...
		if(!empty($_POST['deleteSelect']))
		{
			...
			// var_dump($bw->update('bw_member', $post, "id IN (".$id.")"));
			// return;

			$sql = "UPDATE bw_member SET is_delete1 = 1 WHERE id IN (".$id.")";
	...
	if(!empty($_POST['recoverSelect']))
	{
		...
		$sql = "UPDATE bw_member SET is_delete1 = 0";
		// var_dump($bw->query($sql));
		// var_dump($bw->update('bw_member', $post, ""));
		// return;
...
// die($_SESSION['wheremembertjlist']);
  $_SESSION['wheremembertjlist'] = $_SESSION['wheremembertjlist'].' and is_delete1 != 1';
...
?>
 <tr class="showtr">
    ...
    <td align="center">
    <a href="membermod.php?id=<?php echo $list[$i]['id']?>"><img src="images/pen.gif" alt="修改信息" title="修改信息"></a>
  &nbsp;&nbsp;<a onclick="javascript:if(confirm('是否删除')){window.location.href='?action=delete&id=<?php echo $list[$i]['id']; ?>'}" href="###"><img src="images/delete.gif" />
    </a>
    </td>
  </tr>
  ...
  <tr class="trusername">
    <td colspan="14" align="center">  预存款总额：<?php echo $jyqiantj["zong"];?>&nbsp;&nbsp;总人数：<?php echo $jyqiantj["zongr"];?></td>
        	  ...
        	  <tr>
        	    ...
        	    &nbsp;<input type="submit" value="通过认证" name="sztjBtn" id="sztjBtn" />&nbsp;<input type="submit" value="取消认证" name="qxtjBtn" id="qxtjBtn" />&nbsp;<input type="submit" value="修改审核" name="shenhe" id="shenhe" />&nbsp;<input type="submit" value="删除所选" name="deleteSelect" id="deleteSelect" /><input type="submit" value="恢复所有" name="recoverSelect" id="recoverSelect" /></td>
