...
if($action == 'recover')
{
	...
	if($bw->update('bw_member', $post, 'id = '.$_GET['id']))
	{
		$bw->msg('恢复成功!', 'memberlist.php?page='.$pages.'');
	}else{
		$bw->msg('恢复失败!', 'memberlist.php?page='.$pages.'');
	}
}
...
if(!empty($_POST['recoverSelect']))
{
	...
	if(isset($_POST['id']))
	{
		...
		if(!empty($_POST['recoverSelect']))
		{
			...
			$sql = "UPDATE bw_member SET is_delete1 = 0 WHERE id IN (".$id.")";
        ...
        // var_dump($id);
        // 			return;
                    // if($bw->update('bw_member', $post, "id IN (".$id.")"))
			{
				$bw->msg('恢复成功!', 'memberlist.php?page='.$pages.'');
			}else{
				$bw->msg('恢复失败!', 'memberlist.php?page='.$pages.'');
    ...
    if(isset($_POST['id'])){
        foreach ($_POST['id'] as $key => $value) {
            ...
            switch($ifxj){
				case 1:
				    $vip="一星级";
                    $vip_jifen=$jifenData['yxhy'];
					break;
				case 2:
				    $vip="二星级";
				    $vip_jifen=$jifenData['exhy'];
					break;
				case 3:
				    $vip="三星级";
                    $vip_jifen=$jifenData['sxhy'];
					break;
				case 4:
				    $vip="四星级";
				    $vip_jifen=$jifenData['sixhy'];
					break;
				case 5:
				    $vip="五星级";
                    $vip_jifen=$jifenData['wxhy'];
					break;
				case 6:
				    $vip="普通会员";
				    $vip_jifen=$jifenData['lxhy'];
					break;
            }
            $classData = $bw->selectOnly('*' ,'bw_member', 'id = '.$value);

            $toatl_jifen=intval($classData['total_jifen']+$vip_jifen)-$classData['vip_jifen'];

            $sql = "UPDATE bw_member SET total_jifen=$toatl_jifen,ifxj = $ifxj,vip = '".$vip."',vip_jifen=".$vip_jifen." WHERE id = $value";
            //print_r($sql);exit();
            $bw->query($sql);
        }
...
if(!empty($_POST['sztjBtn']))
{
    ...
    if($bw->query($sql))
    {
        ...
        $jifenData=$bw->selectOnly('*', 'bw_jifenconfig');
        $rz = $jifenData['rz'];
        $sql = "UPDATE bw_member SET rz_jifen=".$rz.",total_jifen=total_jifen+$rz WHERE id IN (".$id.")";
        $bw->query($sql);
        $key = array_keys($_POST['id']);
        if($ii > 0)
		{
			for($i = 0; $i < $ii; $i++)
			{
               $v = $_POST['id'][$key[$i]];
                ...
                $sqlstr="INSERT INTO bw_moneyrecord (memberid,atype,recordno,addtime,acontent,away,amemo,amount,lang,username) VALUES (".$v.",1,'"."Y".date(Ymdhms)."','".date('Y-m-d H:i:s')."',2,'认证费','认证费','-20','".$Lang."','".$_SESSION['username']."')";
                ...
                //从教员存款中扣除认证费20元
                $sqlrzf = "UPDATE bw_member SET Money = Money-20,daoqi='".$daoq."' WHERE id = '".$v."'";
                $bw->query($sqlrzf);
			}
		}
	...
	$id = implode(',', $_POST['id']);
		...
		if($bw->query($sql))
		{
			$jifenData=$bw->selectOnly('*', 'bw_jifenconfig');
			$rz = $jifenData['rz'];
			$sql = "UPDATE bw_member SET total_jifen=total_jifen-$rz WHERE id = $value";
			$bw->query($sql);
	...
	htqx("5.3");
		...
		if($ii > 0)
		{
			...
			$shqk='shqk'.$v;
			    ...
			    if($_POST[$shqk]!="")
				{
				$sql = "update bw_member set shqk=".$_POST[$shqk]."  where id=$v";
				//die($sql);
				$bw->query($sql);
			    }
			}
...
?>
 <tr class="showtr">
    <td align="center"><input type="checkbox" name="id[<?php echo $i;?>]" id="id[]" value="<?php echo $list[$i]['id'];?>" /></td>
    ...
    <td align="center"><a href="javascript:if(confirm('确定要修改密码为123456？')){window.location.href='?action=changepwd&id=<?php echo $list[$i]['id']?>'}" title="重置密码为123456" class="style1">密码重置</a>&nbsp;&nbsp;<a href="membermod.php?id=<?php echo $list[$i]['id'];?>&page=<?php echo $pageArray['page']; ?>"><img src="images/pen.gif" alt="修改信息" title="修改信息" /></a>&nbsp;&nbsp;



    <a onclick="javascript:if(confirm('是否删除')){window.location.href='?action=delete&id=<?php echo $list[$i]['id']; ?>'}" href="###"><img src="images/delete.gif" alt="删除信息" title="删除信息"/></a>

  	<?php
    if($list[$i]['is_delete1'] == 1){
    ?>
    <a onclick="javascript:if(confirm('是否恢复')){window.location.href='?action=recover&id=<?php echo $list[$i]['id']; ?>'}" href="###">恢复</a></td>
  ...
  <tr>
  	<td colspan="14" width="1200" align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        	  <tr>
        	    ...
        	    &nbsp;<input type="submit" value="通过认证" name="sztjBtn" id="sztjBtn" />&nbsp;<input type="submit" value="取消认证" name="qxtjBtn" id="qxtjBtn" />&nbsp;<input type="submit" value="修改审核" name="shenhe" id="shenhe" />&nbsp;<input type="submit" value="删除所选" name="deleteSelect" id="deleteSelect" /><input type="submit" value="恢复所选" name="recoverSelect" id="recoverSelect" /></td>
