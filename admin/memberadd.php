...
if($action == 'insert')
{
	...
	if(empty($id))
	{
    // 增加会员名重复判断 by 昌维 2016-10-30 11:05:50
    if ($bw->sum($bw->query("select * from bw_member where `username`='".$_POST['username']."'"))>0) {
      $bw->msg('新增失败!会员名重复', 'memberlist.php', true);
      return;
    }
    $_POST['levels'] = intval($_POST['levels']);
    ...
    $arr=array('reg_jifen'=>$jifenData['reg']);
    $vip =  $_POST['ifxj'];
    $vip_jifen = 0;
    $vip_name='普通会员';
    if($vip==1){
    ...
    if($vip==4){
        ...
        $arr['vip_jifen'] =$vip_jifen;
        $arr['vip'] =$vip_name;
    $teacherData=array_merge($arr,$_POST);
		//insert($tbName, $post)
		if($bw->insert('bw_member', $teacherData))
		{
...
<table width="100%" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td>
        <input type="hidden" name="reg_action" id="reg_action" value="2" />
        <input type="text" name="username" id="username" value="<?php echo $newsData['username']; ?>" />
...
?>>
  ...
  <tr>
    ...
    <td colspan="7" align="left"><textarea name="xsyq" id="xsyq" cols="45" rows="5" style="width:100%; line-height:20px;">执行武汉上门家教网薪水标准！</textarea></td>
  </tr>
  <tr>
