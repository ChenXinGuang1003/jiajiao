    ...
    if ($ii > 0) {
        for ($i = 0; $i < $ii; $i++) {
            ...
            if ($_POST[$jyidfp] != "") {
                ...
                $sql2 = "UPDATE bw_member SET total_jifen =total_jifen +".$jifenData['success_order']." ,success_jifen = success_jifen+" . $jifenData['success_order'] . " where id=" . $_POST[$jyidfp];
                $bw->query($sql);
                $bw->query($sql1);
                $bw->query($sql2);
            }
...
<table width="96%" cellpadding="0" cellspacing="0">
    ...
    <form name="listForm" action="?action=deleteFrom" method="post">
        ...
        $where = "1=1 and zt='未安排' and isshow=2";
        //搜索
        ...
        for ($i = 0; $i < $sum; $i++) {
            ?>
            <tr class="showtr">
                <td align="center"><input type="checkbox" name="id[]" id="id[]" value="<?php echo $list[$i]['id']; ?>"/>
                </td>
                <td align="center"><a
                        href="xyadd.php?id=<?php echo $list[$i]['id'] ?>"><?php echo $list[$i]['id']; ?></a></td>
                <td align="center"><a
                        href="xyadd.php?id=<?php echo $list[$i]['id'] ?>"><?php echo $list[$i]['name']; ?></a></td>
                <td align="center"><?php echo $list[$i]['xynj']; ?></td>
                <td align="center"><?php echo $list[$i]['xylx']; ?></td>
                <td align="center"><a href="###"
                                      title="<?php echo $list[$i]['qjkm']; ?>"><?php echo $list[$i]['qjkm']; ?></a></td>
                <td align="center"><?php echo $list[$i]['szqy']; ?></td>
                <td align="center"><?php if ($list[$i]['zt'] == "正在试教") {
                        echo "<font color='#FF0000'>" . $list[$i]['zt'] . "</font>";
                    } else {
                        echo $list[$i]['zt'];
                    } ?></td>
                <td align="center"><input name="xxf<?php echo $list[$i]['id']; ?>" type="text" id="xxf"
                                          value="<?php echo $list[$i]['xxf']; ?>" size="10"/></td>
                ...
                <td align="center">
                    <!--<a href="?action=ggzt&id=<?php echo $list[$i]['id'] ?>"><img src="images/pen.gif" alt="更改状态" title="更改状态" />--></a>
                    <a href="xyadd.php?id=<?php echo $list[$i]['id'] ?>"><img src="images/pen.gif" alt="修改信息"
                                                                              title="修改信息"/></a>&nbsp;&nbsp;<a
                        href="###"
                        onclick="javascript:if(confirm('是否删除')){window.location.href='?action=delete&id=<?php echo $list[$i]['id']; ?>'}"><img
        ...
        <tr>
            <td colspan="12" align="center">
                <div class="pageDiv">
                    <div class="pageLeft" style="width:450px;"><input type="button" value="全选"
                                                                      id="selectAll"/>&nbsp;<input type="button"
                                                                                                   value="反选"
                                                                                                   id="orderSelect"/>&nbsp;<input
                            type="submit" value="删除所选" name="deleteSelect" id="deleteSelect"/>&nbsp; <input
                            type="submit" value="分配教员" name="fenpei" id="fenpei"/>&nbsp;<input type="submit"
                                                                                               value="更改信息费"
                                                                                               name="shsyBtn"
                                                                                               id="shsyBtn"/></div>
                    <div class="pageRight" style="width:500px;">
                        共:<?php echo $pageArray['totalRow']; ?>
                        &nbsp;条信息&nbsp;当前:<span><?php echo $pageArray['page']; ?></span>/<?php echo $pageArray['totalPage']; ?>
                        页&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href="?page=1">第一页</a>&nbsp;&nbsp;
