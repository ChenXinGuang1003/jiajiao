...
<script type="text/javascript">
	$(document).ready(function(){
		...
		location.href="student_yyjy.php?page="+thisPage.val()"";
		});//end page_SEL 		
	});
</script>
</HEAD>
...
if($_GET["act"]=="yy"&&!empty($_GET["id"]))
{
	...
	{
	$bw->msg('取消成功', 'student_yyjy.php');
	exit;
	}else{
	$bw->msg('取消失败', 'student_yyjy.php');
	exit;
		}
}
...
<div class="user_right">
     ...
     <div class="sjgl_memu2">预约教员</div>
    </div>
    <div id="sjgl_nr">
	  <table width="100%" id="sj" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
        <tr>
          <td width="16%" height="29" align="center" bgcolor="#FFFFFF" class="tdbg">订单时间</td>
          <td width="20%" align="center" bgcolor="#FFFFFF" class="tdbg">教员姓名</td>
		  ...
		  <td width="4%" align="center" bgcolor="#FFFFFF" class="tdbg">管理</td>
        </tr>
		<?php
  //查询
...
?>
		...
		<td align="center" bgcolor="#FFFFFF"><input type="button" name="button" id="button" value="取消预约" onClick="javascript:if(confirm('您确定要取消预约吗？')){window.location.href='?act=yy&id=<?php echo $list[$i]['id']; ?>'}"></td>
        </tr>
		<?php
	}//end loop
?>
        ...
        <tr>
         <td height="100" colspan="7" bgcolor="#FFFFFF" style="line-height:20px;">
		  <?php
